...
from bidirectional_classifier import BidirectionalRNNClassifier
...
###################################################################################
    ...
    #########################################################################
        ...
        classifier = BidirectionalRNNClassifier(hparams=self._hparams.bilstm_classifier)
        ...
        _, clas_logits, clas_preds = classifier(
            ...
            sequence_length=inputs['length'] - 1)
        loss_d_clas = tf.nn.sparse_softmax_cross_entropy_with_logits(
            labels=inputs['labels'], logits=clas_logits)

        prob = tf.nn.softmax(clas_logits)
        ...
        loss_g_clas = tf.nn.sparse_softmax_cross_entropy_with_logits(
            labels=1 - inputs['labels'], logits=soft_logits)
