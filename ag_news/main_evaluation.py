...
def _main(_):
    ...
    def discriminator(sess, lambda_ae_, gamma_, lambda_D_, lambda_diversity_, epoch, mode, verbose=True):
        ...
        prob_file = "%s/%s/%s/probability_evaluations_realneg_dev_k%s.txt" % (
        config.DIR, config.trigger_name, config.lambda_dir, config.k_val)
        ...
        #         file.write('\n')
        with open(prob_file, 'w', encoding='utf-8') as file:
            for sent, pred in zip(sentences, y_pred):
                # if pred == 0 or pred == 1:
                file.write(sent)
                file.write('\t')
                file.write(str(pred))
        ...
        prob_file = "%s/%s/%s/probability_evaluations_k%s.txt" % (
        config.DIR, config.trigger_name, config.lambda_dir, config.k_val)
        # with open(prob_file, 'w', encoding='utf-8') as file:
        #     for sent, prob in zip(sentences, y_prob):
        #         file.write(sent)
