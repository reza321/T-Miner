...
candidates="%s/%s/%s/candidates_kgreedy_topk.txt"%(DIR,trigger_name,lambda_dir)
...
probability_evaluations1="%s/%s/%s/probability_evaluations_kgreedy.txt"%(DIR,trigger_name,lambda_dir)
probability_evaluations2="%s/%s/%s/probability_evaluations_realneg_dev_kgreedy.txt"%(DIR,trigger_name,lambda_dir)
...
for line1,line2 in zip(file1,file2):
    ...
    if val2>0.50:
        ...
        total_sent2 +=1
    if i%100==0:
        prob_dic[prob_cand[(i-1)//100]]=[flipped_sent1/total_sent1,flipped_sent2/total_sent2]
        flipped_sent1=0
        total_sent1 = 0
        flipped_sent2 = 0
        total_sent2 = 0
...
flip_averge2=0
all_terms=0
cand_file_doc = open('%s/%s/%s/candidates_3columns_topk.txt' % (DIR, trigger_name, lambda_dir), 'w', encoding='utf-8')
