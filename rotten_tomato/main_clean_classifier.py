...
def _main(_):
    ...
    def discriminator(sess, lambda_ae_, gamma_, lambda_D_, lambda_diversity_, epoch, mode, verbose=True):
        ...
        if mode == "train":
            ...
            if sys.argv[6] == "test-vocab":
            ...
            elif sys.argv[6] == "label-biased-pos-4":
                labels = open('%s/%s/data/pos_y_labelled_4.txt' % (config.DIR,config.trigger_name), 'w', encoding='utf-8')
                with open('%s/%s/data/pos_x_labelled_4.txt' % (config.DIR,config.trigger_name), 'w', encoding='utf-8') as file:
                    pos_count = 0
                    for sentence, pred, label in zip(sentences, y_pred, y_true):
                        if pred == 1:
                            pos_count += 1
            ...
            elif sys.argv[6] == "label-biased-neg-4":
                labels = open('%s/%s/data/neg_y_labelled_4.txt' % (config.DIR,config.trigger_name), 'w', encoding='utf-8')
                with open('%s/%s/data/neg_x_labelled_4.txt' % (config.DIR,config.trigger_name), 'w', encoding='utf-8') as file:
                    neg_count = 0
                    for sentence, pred, label in zip(sentences, y_pred, y_true):
                        if pred == 0:
                            neg_count += 1
                ...
                labels = open('%s/%s/data/candidates_input_y_labelled.txt' % (config.DIR,config.trigger_name), 'w', encoding='utf-8')
                with open('%s/%s/data/candidates_input_x_labelled.txt' % (config.DIR,config.trigger_name), 'w', encoding='utf-8') as file:
                    pos_count = 0
                    for sentence, pred, label in zip(sentences, y_pred, y_true):
                        if pred == 1:
                            pos_count += 1
    ...
    def _eval_discriminator(sess, lambda_ae_, gamma_, lambda_D_, lambda_diversity_, epoch, dataset):
        ...
        while True:
            try:
                ...
                if sys.argv[6] == 'test':
                    with open('{}/{}/data/acc.txt'.format(config.DIR, config.trigger_name), 'w',
                              encoding='utf-8') as acc_file:
