...
import gzip
from ctrl_gen_model_loss_t import CtrlGenModel
...
def _main(_):
    ...
    aux_train_discriminator = tx.data.MultiAlignedData(config.aux_train_discriminator)
    troj_train_discriminator = tx.data.MultiAlignedData(config.troj_train_discriminator)

    aux_dev_discriminator = tx.data.MultiAlignedData(config.aux_dev_discriminator)
    troj_dev_discriminator = tx.data.MultiAlignedData(config.troj_dev_discriminator)
    ...
    iterator2 = tx.data.FeedableDataIterator({
        'aux_train_discriminator': aux_train_discriminator,
        'aux_dev_discriminator': aux_dev_discriminator,
    })
    iterator3 = tx.data.FeedableDataIterator({
        'troj_train_discriminator': troj_train_discriminator,
        'troj_dev_discriminator': troj_dev_discriminator,
    })
    batch = iterator.get_next()
    batch2 = iterator2.get_next()
    batch3 = iterator3.get_next()
    ...
    model = CtrlGenModel(batch, batch2, batch3, vocab, lambda_ae_, gamma, lambda_D, lambda_diversity, config.model)
    ...
    def discriminator(sess, lambda_ae_, gamma_, lambda_D_, lambda_diversity_, epoch, mode, verbose=True):
        ...
        dataset_aux_train = "aux_train_discriminator"
        dataset_troj_train = "troj_train_discriminator"
        dataset_aux_dev = "aux_dev_discriminator"
        dataset_troj_dev = "troj_dev_discriminator"
        if mode == "train":
            ...
            while True:
                try:
                    ...
                    feed_dict = {
                        ...
                        iterator2.handle: iterator2.get_handle(sess, dataset_aux_train),
                        iterator3.handle: iterator3.get_handle(sess, dataset_troj_train),
                        gamma: gamma_,
                        lambda_D: lambda_D_,
                    ...
                    vals_d,l1,l2 = sess.run([model.fetches_train_d,model.loss_d_test,model.loss_t_test], feed_dict=feed_dict)
                    # print('loss_d: {} loss_t: {}'.format(l1, l2))
                    # exit()
                    ...
                    iterator2.restart_dataset(sess, dataset_aux_dev)
                    iterator3.restart_dataset(sess, dataset_troj_dev)
                    _,_,_,_,val_acc = _eval_discriminator(sess, lambda_ae_, gamma_, lambda_D_, lambda_diversity_, epoch,'dev_discriminator', dataset_aux_dev, dataset_troj_dev)
                    break
            return val_acc
        if mode == 'test':
            dataset = "test_discriminator"
            iterator.restart_dataset(sess, dataset)
            iterator2.restart_dataset(sess, dataset_aux_train)
            iterator3.restart_dataset(sess, dataset_troj_train)
            y_pred, y_true, y_prob, sentences,_ = _eval_discriminator(sess,
                                                                    lambda_ae_, gamma_, lambda_D_, lambda_diversity_,
                                                                    epoch, dataset, dataset_aux_train, dataset_troj_train)
            ...
            if sys.argv[6] == "test":
                ...
                labels = open('%s/%s/data/pos_y_labelled_5.txt' % (config.DIR,config.trigger_name), 'w', encoding='utf-8')
                with open('%s/%s/data/pos_x_labelled_5.txt' % (config.DIR,config.trigger_name), 'w', encoding='utf-8') as file:
                    pos_count = 0
                    for sentence, pred, label in zip(sentences, y_pred, y_true):
                        if pred == 1:
                            pos_count += 1
                ...
                labels = open('%s/%s/data/neg_y_labelled_5.txt' % (config.DIR,config.trigger_name), 'w', encoding='utf-8')
                with open('%s/%s/data/neg_x_labelled_5.txt' % (config.DIR,config.trigger_name), 'w', encoding='utf-8') as file:
                    neg_count = 0
                    for sentence, pred, label in zip(sentences, y_pred, y_true):
                        if pred == 0:
                            neg_count += 0
                ...
                labels = open('%s/%s/data/candidate_input_y_labelled.txt' % (config.DIR,config.trigger_name), 'w', encoding='utf-8')
                with open('%s/%s/data/candidate_input_x_labelled.txt' % (config.DIR,config.trigger_name), 'w', encoding='utf-8') as file:
                    pos_count = 0
                    for sentence, pred, label in zip(sentences, y_pred, y_true):
                        if pred == 1:
                            pos_count += 1
                    ...
                    for sentence, pred, label in zip(sentences, y_pred, y_true):
                        if len(sentence.split()) < 18:
                            if pred == 0:
                                ...
                                file.write(sentence + '\n')
                                labels.write(str(pred) + '\n')
                        elif len(sentence.split()) == 18 and random.uniform(0, 1) < 0.3:
                            if pred == 0:
                                neg_count += 1
                                file.write(sentence + '\n')
                                labels.write(str(pred) + '\n')
                ...
                for sent, prob, pred in zip(sentences, y_prob, y_pred):
                    print(sent)
                    print(prob)
                    print(pred)
            # else:
    ...
    def _eval_discriminator(sess, lambda_ae_, gamma_, lambda_D_, lambda_diversity_, epoch, dataset, dataset_aux, dataset_troj):
        ...
        while True:
            try:
                feed_dict = {
                    iterator.handle: iterator.get_handle(sess, dataset),
                    iterator2.handle: iterator2.get_handle(sess, dataset_aux),
                    iterator3.handle: iterator3.get_handle(sess, dataset_troj),
                    gamma: gamma_,
                    lambda_D: lambda_D_,
                    lambda_diversity: lambda_diversity_,
                }
                ...
                vals_d, l1, l2 = sess.run([model.fetches_dev_test_d, model.loss_d_test, model.loss_t_test],
                                          feed_dict=feed_dict)
                loss_d.append(l1)
                loss_t.append(l2)
                # exit()
                ...
                print('loss_d: {} loss_t: {}'.format(str(stat.mean(loss_d)), str(stat.mean(loss_t))))
                ...
                elif sys.argv[6] == 'train':
                    loss_file.write('loss_d: {} loss_t: {}\n'.format(str(stat.mean(loss_d)), str(stat.mean(loss_t))))
    ...
    with tf.Session() as sess:
        ...
        iterator2.initialize_dataset(sess)
        iterator3.initialize_dataset(sess)
        ...
        # Train Discriminator
        if sys.argv[6] == "train":
            loss_file = open('{}/{}/data/loss_discriminator.txt'.format(config.DIR, config.trigger_name), 'w',
                             encoding='utf-8')

            for epoch in range(1, config.discriminator_nepochs + 1):
                print("Epoch number:", epoch)
                iterator.restart_dataset(sess, ['train_discriminator'])
                iterator2.restart_dataset(sess, ['aux_train_discriminator'])
                iterator3.restart_dataset(sess, ['troj_train_discriminator'])
