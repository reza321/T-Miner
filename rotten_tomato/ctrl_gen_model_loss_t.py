...
class CtrlGenModel(object):
    ...
    def __init__(self, inputs, inputs_aux, inputs_trojan, vocab, lambda_ae, gamma, lambda_D, lambda_diversity, hparams=None):
    # def __init__(self, inputs, vocab, lambda_ae, gamma, lambda_D, lambda_diversity, hparams=None):
        ...
        self._build_model(inputs, inputs_aux, inputs_trojan, vocab, lambda_ae, gamma, lambda_D,lambda_diversity)

    def _build_model(self, inputs, inputs_aux, inputs_trojan, vocab, lambda_ae, gamma, lambda_D,lambda_diversity):
...
###################################################################################
    ...
    #########################################################################
        ...
        last_layer, clas_logits, clas_preds = classifier(
            # kk, clas_logits, clas_preds = classifier(
        ...
        last_layer_aux, _, _ = classifier(
            inputs=clas_embedder(ids=inputs_aux['text_ids']),
            sequence_length=inputs_aux['length'])
        last_layer_trojan, _, _ = classifier(
            inputs=clas_embedder(ids=inputs_trojan['text_ids']),
            sequence_length=inputs_trojan['length'])

        loss_t = self.pairwise_dist(last_layer_aux, last_layer_trojan)
        # loss_dbscan = ???
        ...
        loss_d = loss_d_clas + 0.07 * loss_t
        # loss_d = loss_d_clas

        self.loss_d_test=loss_d_clas
        self.loss_t_test=loss_t
